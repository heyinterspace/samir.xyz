...
echo "Starting parallel Next.js build and simple server..."
...
# Start the simple server in background using nohup to ensure it keeps running
echo "Starting simple server on port 5000..."
nohup bun run simple-server.js > server.log 2>&1 &
simple_server_pid=$!
