...
export function ProjectCard({ name, description, imageUrl, link, priority = false }: ProjectCardProps) {
  return (
    <Link 
      href={link}
      target="_blank"
      className="group relative overflow-hidden rounded-xl bg-white dark:bg-gray-800 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
    >
      <div className="relative aspect-square w-full overflow-hidden">
        <Image
          ...
          alt={name}
          fill
          className="object-contain p-8 transform group-hover:scale-105 transition-transform duration-300"
