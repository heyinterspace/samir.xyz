 ...
 * with 2 rows of 4 KPIs as requested - Shadcn UI version
 */
export default function StatsSection() {
  return (
    <Card className="w-full max-w-[800px] mb-8">
          ...
          {statsData.map((row, rowIndex) => (
            <div key={`row-${rowIndex}`} className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {row.map((stat) => (
                <div 
                  key={stat.label} 
                  className="p-3 rounded-md bg-secondary/40 hover:bg-secondary/60 transition-all duration-200 
                         flex flex-col items-center text-center transform hover:-translate-y-0.5"
                >
                  <h3 className="text-sm font-medium text-muted-foreground mb-1 w-full overflow-hidden text-ellipsis whitespace-nowrap">
                    {stat.label}
                  </h3>
